...
import numpy as np
import datetime
from characters import barbarians, dragons, balloons, archers, sneaky_archers
...
def sneakyArchColor(sneaky_arch):
    health = sneaky_arch.health
    max_health = sneaky_arch.max_health
    percentage = (health*100)/max_health
    if(percentage > 50):
        return Back.LIGHTMAGENTA_EX
    elif percentage > 20:
...
def printMap(V):
    ...
    for sneaky in sneaky_archers:
        a = 2*sneaky.position[0]
        b = 2*sneaky.position[1]
        map_matrix[a][b] = sneakyColor(sneaky) + Fore.BLACK + 'SN'
        map_matrix[a+1][b] = sneakyColor(sneaky) + Fore.BLACK + '  '
        map_matrix[a][b+1] = sneakyColor(sneaky) + Fore.BLACK + 'EA'
        map_matrix[a+1][b+1] = sneakyColor(sneaky) + Fore.BLACK + 'KY'
