...
def spawnSneakyArcher(pos):
    if(pt.troop_limit['sneaky_archer'] <= troops_spawned['sneaky_archer']):
        return

    # convert tuple to list
    pos = list(pos)
    sneaky_archer = SneakyArcher(pos)
    troops_spawned['sneaky_archer'] += 1
    sneaky_archers.append(sneaky_archer)
...
def move_sneaky_archers(V,type):
    if(type == 1):
        for sneaky in sneaky_archers:
            if(sneaky.alive == False):
                continue
            if(sneaky.target != None):
                if(V.map[sneaky.target[0]][sneaky.target[1]] == pt.BLANK):
                    sneaky.target = None
            if(sneaky.target == None):
                sneaky.target = search_for_closest_building(sneaky.position, V.map, 0)
            if(sneaky.target == None):
                continue
            sneaky.move(sneaky.target, V, type)
    elif(type == 2):
        for sneaky in sneaky_archers:
            if(sneaky.alive == False):
                continue
            closest_building = search_for_closest_building(sneaky.position, V.map, 0)
            if(closest_building == None):
                continue
            sneaky.move(closest_building, V, type)
